...
def contact(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            query = "{0} has sent you a new message:\n\n{1}".format(name, form.cleaned_data['query'])
            send_mail('New Enquiry', query, email, ['@@@@@@@@']) # Own Email Id where you want to recive mail from user
